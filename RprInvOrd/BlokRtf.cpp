...
//AfxMessageBox("Нашёл в GetStrWrtStr \n"+strTg);
		...
		curPos+= iCnt;
		s.Format(_T(" Нашёл GetStrWrtStr curPos = %i"),curPos);
AfxMessageBox(s+'\n'+strTg);
...
//		AfxMessageBox(s);
		...
		AfxMessageBox(_T("Не нашёл GetRStrWrt\n")+strBuf);
...
//AfxMessageBox("Нашёл в GetR \n"+strTg);
		iCnt = strTg.GetLength();
		strOut+=strTg;
		s.Format(_T(" Нашёл GetRStr curPos = %i"),curPos);
AfxMessageBox(_T("Нашёл в GetRStr \n")+s+'\n'+_T("strTg = ")+strTg);
		curPos+= iCnt;
		inF.Seek(curPos,CFile::begin); // поставил указатель ЗА
//			s.Format(" после нашёл Seek curPos = %i",curPos);
//		AfxMessageBox(s);
...
void CBlokRtf::Control(CFile &f, int iBlk)
{
	...
	m.Format(_T("Тек поз curPos = %i"),curPos);
AfxMessageBox(m);
	...
	m.Format(_T("Поз после Read curPos = %i прочитано iR = %i"),curPos,iR);
AfxMessageBox(m);
	CString s=_T("");
	for(int i=0;i<iBlk;i++){
		if(_T(' ')==*(b+i) ){
...
//	curPos-=iBlk;
	...
	m.Format(_T("Поз для возврата curPos = %i"),curPos);
AfxMessageBox(m);

	f.Seek(curPos,CFile::begin);

	curPos = f.GetPosition();
	m.Format(_T("Тек поз после Seek назад на исх curPos = %i"),curPos);
AfxMessageBox(m);
	delete [] b;
