...
BOOL CDlg::OnInitDialog()
{
	...
	AfxSetResourceHandle(::GetModuleHandle(L"TNDC.dll"));
...
*/
	...
	strSql = _T("QT64 ");
	strSql+= strC;
	OnShowGrid(strSql, ptrRs1,ptrCmd1,m_DataGrid1,InitDataGrid1,0,3);
	if(m_bFndC){
		short fCol=0;
		m_Flg = false;
	...
	strSql = _T("QT63Excld");
	OnShowGrid(strSql, ptrRs3,ptrCmd3,m_DataGrid3,InitDataGrid3,0,2);
...
//Добавить
//	UpdateData();
	CString strSql,strC2,strC3,strC;
	_variant_t vra;
	VARIANT* vtl = NULL;
	short i,IndCol;
	COleVariant vC;
...
//			AfxMessageBox(strSql);
		...
		if(IsEnableRec(ptrRs1)){
			IndCol = 1;
			m_Flg = false;
			OnFindInGrid(strC2,ptrRs1,IndCol,m_Flg);
			m_Flg = true;
		}
	}
...
// Изменить
//	UpdateData();
	CString strSql,strC,strC2,strC3;
	_variant_t vra;
	COleVariant vC;
	VARIANT* vtl = NULL;
	short IndCol,i;
...
BEGIN_EVENTSINK_MAP(CDlg, CDialog)
	...
	ON_EVENT(CDlg, IDC_DATAGRID1, DISPID_CLICK, ClickDatagrid1, VTS_NONE)
	ON_EVENT(CDlg, IDC_DATAGRID2, DISPID_CLICK, ClickDatagrid2, VTS_NONE)
	ON_EVENT(CDlg, IDC_DATAGRID3, DISPID_CLICK, ClickDatagrid3, VTS_NONE)
END_EVENTSINK_MAP()
