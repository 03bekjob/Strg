...
BOOL CDlg::OnInitDialog()
{
	...
	AfxSetResourceHandle(::GetModuleHandle(L"NameStrt.dll"));
...
*/
	...
	try{
		...
		m_DataCombo1.put_ListField(_T("Полн. наим."));
		if(m_bFndC){
			m_Flg = false;
			OnFindInCombo(m_strFndC,&m_DataCombo1,ptrRsCmb1,0,2);
//			m_Flg = true;
		}
	}
	catch(_com_error& e){
	...
	if(m_bFndC){
		if(IsEnableRec(ptrRs1)){
			short fCol=0;
			m_Flg = false;
			OnFindInGrid(m_strFndC1,ptrRs1,fCol,m_Flg);
			m_Flg = true;
		}
	}
...
//Добавить
	...
	if(OnOverEdit(IDC_EDIT1,IDC_EDIT1)){
		IndCol = 0;
		vC=GetValueRec(ptrRsCmb1,IndCol);
		vC.ChangeType(VT_BSTR);
		strC=vC.bstrVal;
		strC.TrimLeft(' ');
		strC.TrimRight(' ');
...
void CDlg::ChangeDatacombo1()
{
	long cd=0;
	if(m_Flg){
		cd = OnChangeCombo(m_DataCombo1,ptrRsCmb1,2);
	}
	if(IsEnableRec(ptrRsCmb1)){
		OnShowName(cd);
	}
...
void CDlg::OnShowName(long cd)
{
	CString strSql;
	if(cd!=0){
		CString strCd,strSql;
