...
BOOL CDlg::OnInitDialog()
{
	...
	AfxSetResourceHandle(::GetModuleHandle(L"NDC.dll"));
...
*/
	...
	strSql = _T("QT63");
	OnShowGrid(strSql, ptrRs1,ptrCmd1,m_DataGrid1,InitDataGrid1,0,2);
	if(m_bFndC){
		short fCol=0;
		m_Flg = false;
...
//AfxMessageBox(s);
		...
		if(m_CurCol==-1 || m_CurCol==0 || m_CurCol==1){
			m_CurCol = 2;
		}
...
//			AfxMessageBox(strSql);
		...
		if(IsEnableRec(ptrRs1)){
			if(!m_Edit1.IsEmpty()){
				IndCol = 1;
				m_Flg = false;
				OnFindInGrid(strD,ptrRs1,IndCol,m_Flg);
				m_Flg = true;
			}
		}
	}
...
// Изменить
	UpdateData();
	CString strSql,strC,strD,strCh;
	_variant_t vra;
	COleVariant vC;
	VARIANT* vtl = NULL;
	short IndCol,i;
...
void CDlg::OnShowGrid(CString strSql,_RecordsetPtr& rs,_CommandPtr& Cmd,CDatagrid1& Grd,void (*pFGrd)(CDatagrid1&,_RecordsetPtr&),int EmpCol,int DefCol)
{
	...
	//		s.Format("%i",m_CurCol);
	//		AfxMessageBox(s);
			if(m_CurCol==-1 || m_CurCol==EmpCol || m_CurCol==1){
				m_CurCol = DefCol;
			}
