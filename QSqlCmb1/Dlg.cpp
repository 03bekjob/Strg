...
BOOL CDlg::OnInitDialog()
{
	...
	AfxSetResourceHandle(::GetModuleHandle(L"QSqlCmb1.dll"));
...
//	OnShowQuery(m_lCdQr);
	CString strCap;
	strCap.LoadString(IDS_STRING9014);
...
void CDlg::OnClickedOk()
{
	...
	CString s;
	CString strCdQr,strC2,strC3,strC9;
	strCdQr.Format(_T("%i"),m_lCdQr);

	s = m_DataCombo1.get_BoundText();
	s.TrimLeft();
	s.TrimRight();
	strC9 = m_OleDateTime1.Format(_T("%Y-%m-%d"));
	strC2=strC3=_T("0");
...
*/
	...
	case 3:
		...
		break;
	case 4:
		strSql=	_T("QT12_1");			// Товар
		GetDlgItem(IDC_DATETIMEPICKER1)->ShowWindow(false);
		GetDlgItem(IDC_STATIC_DATETIME1)->ShowWindow(false);
		break;
	case 5:
		strSql=	_T("QT12_1");			// Товар+Дата
		GetDlgItem(IDC_DATETIMEPICKER1)->ShowWindow(true);
		GetDlgItem(IDC_STATIC_DATETIME1)->ShowWindow(true);
		break;
	}
